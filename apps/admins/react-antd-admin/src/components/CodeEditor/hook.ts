...
async function loadLinterExtension(language: string): Promise<Extension | null> {
  ...
  try {
    ...
    console.debug(`Failed to load linter for ${language}`, error as Error)
    return null
  }
}
 ...
 * 加载语言相关扩展
 */
export function useLanguageExtensions(language: string, lint?: boolean) {
 ...
 * @param {UseBlurHandlerProps.onBlur} onBlur blur 事件触发时的回调函数
 * @returns 扩展或空数组
 */
 ...
 * @param {UseHeightListenerProps.onHeightChange} onHeightChange 高度变化时触发的回调函数
 * @returns 扩展或空数组
 */
